...
class ComboWidget extends React.Component {
  ...
  get initialStateEvents() {
    return this.props.events.map((event, i) => ({
      ...
      get selectedOutcome() {
        return this.event.betOffers.outcomes[this.selectedOutcomeIdx]
      },
    }))
  }
  ...
  selectOutcome(outcome, event) {
    this.state.events[
      this.props.events.indexOf(event)
    ].selectedOutcomeIdx = event.betOffers.outcomes.indexOf(outcome)
    this.setState({ events: this.state.events }, this.renderCombinedOdds)
  }
   ...
   */
  renderCombinedOdds() {
    getFormattedOdds(this.combinedOdds * 1000).then(combinedOdds =>
      this.setState({ combinedOdds })
    )
  }
      ...
      <div>
        ...
        <TopBar>
          <ActionButton
            ...
            type="secondary"
            disabled={
              this.visibleEvents.length >= this.props.selectionLimit ||
              this.props.events.length <= this.props.selectionLimit
            }
          >
            {t('Add')}
          </ActionButton>
          <ActionButton action={this.resetEvents.bind(this)} type="secondary">
            {t('Reset')}
          </ActionButton>
        </TopBar>
